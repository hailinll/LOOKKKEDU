...
class OpAdmission(models.Model):
    ...
    def write(self, vals):
        for record in self:
            synced = record._sync_cn_name_vals(dict(vals), current_record=record)
            super(OpAdmission, record).write(synced)
        return True

    def _sync_cn_name_vals(self, vals, current_record=None):
        """兼容中文姓名与英文拆分字段，保证主名称一致。"""
        ...
        curr_fname = fname if fname is not None else (current_record.first_name if current_record else False)
        curr_mname = mname if mname is not None else (current_record.middle_name if current_record else False)
        curr_lname = lname if lname is not None else (current_record.last_name if current_record else False)
        merged = " ".join(filter(None, [curr_fname, curr_mname, curr_lname])).strip()
        if merged:
    ...
    def _onchange_name_cn(self):
        for rec in self:
            if rec.name_cn:
                rec.name = rec.name_cn
                rec.first_name = rec.name_cn
                rec.middle_name = False
                rec.last_name = False
    ...
    def onchange_student(self):
        if self.is_student and self.student_id:
            ...
            self.name_cn = sd.name_cn or sd.name
